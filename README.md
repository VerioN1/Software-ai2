...
- after Updating the schema: run : 1)prisma db push, 2)prisma generate, 3)npm run build
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create stms-v2-3a48
  fly apps create stms-v2-3a48-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app stms-v2-3a48
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app stms-v2-3a48-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'stms-v2-3a48-staging' does not match app name in config file 'stms-v2-3a48'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name stms-v2-3a48-db
  fly postgres attach --app stms-v2-3a48 stms-v2-3a48-db

  fly postgres create --name stms-v2-3a48-staging-db
  fly postgres attach --app stms-v2-3a48-staging stms-v2-3a48-staging-db
  ```
